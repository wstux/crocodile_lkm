...
There are not hidden processes"
    ...
    # check data
    expect_qe "crok__proc_fs" etalon read_data

    unload_device
}

function crok__log_level
{
    local etalon="6"
    ...
    # check data
    expect_qe "crok__log_level::default" etalon log_level

    # Check debug log level
    change_log_level "DEBUG"
    etalon="7"
    log_level="$( read_log_level )"
    expect_qe "crok__log_level::debug" etalon log_level

    # Check critical log level
    change_log_level "CRIT"
    etalon="2"
    log_level="$( read_log_level )"
    expect_qe "crok__log_level::crit" etalon log_level

    unload_device
}

function crok__hide_proc
{
...
Hidden pids:
  -- ${pid};"
    ...
    expect_true "crok__hide_proc::running_process" is_proc_exist

    if [[ "${is_proc_exist}" == "true" ]]; then
        # hide process
        hide_pid "${pid}"
        is_proc_exist="$( is_process_running "${pid}" )"
        expect_false "crok__hide_proc::hidden_process" is_proc_exist

        # read from proc fs
        local read_data="$( cat "/proc/$( get_device_name )" )"
        expect_qe "crok__hide_proc::pid_in_proc_fs" etalon read_data

        # show process
        show_pid "${pid}"
        is_proc_exist="$( is_process_running "${pid}" )"
        expect_true "crok__hide_proc::showed_process" is_proc_exist
        if [[ "${is_proc_exist}" == "true" ]]; then
            stop_process "${pid}"
        fi
    fi
